...
void infinity_Board::remove_oldest_move() {
    if (!history.empty()) {
        auto oldest_move = history.front();
        history.pop();

        int x = oldest_move.first;
        int y = oldest_move.second;
...
infinity_UI::infinity_UI() : UI<char>("Weclome to FCAI X-O Game by Dr El-Ramly", 3) {}
